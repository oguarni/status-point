...
* **RN03 \- Fluxo de Status:** As tarefas devem seguir um fluxo lógico de estados (todo \-\> in\_progress \-\> completed), embora o sistema permita flexibilidade via Kanban.  
...
1. **Nível 1 (Contexto):** Demonstra a interação do Usuário com o Sistema Agiliza.  
2. **Nível 2 (Contêineres):** Detalha a separação entre:  
   * *Single Page Application* (React/Vite).  
   * *API Application* (Node.js/Express).  
   * *Database* (PostgreSQL).  
3. **Nível 3 (Componentes):** Explode o Backend API para mostrar as camadas da Clean Architecture:  
   * Controllers (Adaptadores de Interface).  
   * Use Cases / Services (Regras de Negócio da Aplicação).  
   * Entities (Regras de Negócio da Empresa).  
   * Repositories (Acesso a Dados).

### **4.2. Diagrama de Banco de Dados (ER)**

O esquema relacional implementado no PostgreSQL consiste nas seguintes entidades principais:

* **Users:** id, name, email, password\_hash, role  
* **Projects:** id, title, description, deadline, gestor\_id  
* **Tasks:** id, title, status, priority, user\_id, project\_id, assignee\_id  
* **TaskHistory:** Rastreabilidade de mudanças de status.  
* **TaskComments / TaskAttachments:** Entidades de suporte à colaboração.

## **5\. Semana 4 e 5: Desenvolvimento do MVP (Entregáveis)**

O desenvolvimento atingiu e superou a meta de 5 funcionalidades completas. O código está versionado no GitHub e organizado em monorepo (backend/ e frontend/).

### **Funcionalidades Entregues (Status: ✅ Concluído)**

1. **Sistema de Login e Registro Seguro:**  
   * *Backend:* AuthService, AuthController, authMiddleware, validações com express-validator.  
   * *Frontend:* Telas de Login/Registro responsivas com feedback de erro.  
2. **CRUD de Projetos com Validação de Papel:**  
   * *Backend:* Middleware requireGestorOrAdmin garante que apenas gestores criem projetos.  
   * *Frontend:* Listagem de projetos com *badges* de status (Urgente/Atrasado).  
3. **Quadro Kanban Interativo:**  
   * *Frontend:* Implementação completa de *Drag-and-Drop* para mover tarefas entre colunas.  
   * *Backend:* Endpoint /api/tasks/kanban otimizado para essa visualização.  
4. **Sistema de Busca e Filtros Avançados:**  
   * Implementação de filtros dinâmicos (SQL ILIKE e WHERE condicionais) para buscar tarefas por texto, prioridade ou status.  
5. **Anexos e Uploads:**  
   * Implementação de upload de arquivos utilizando Multer no backend, salvando metadados no banco e arquivos em disco.  
6. **Internacionalização:**  
   * Sistema completo de troca de idiomas (PT-BR / EN) em tempo real no frontend.

## **6\. Procedimentos de Operação (Deploy)**

O projeto foi configurado para rodar inteiramente via **Docker**, facilitando a operação e o deploy em qualquer ambiente que suporte contêineres.

### **Pré-requisitos**

* Docker e Docker Compose instalados.

### **Como Rodar (Ambiente de Produção/Local)**

1. **Clonar o repositório:**  
   git clone \[https://github.com/oguarni/status-point.git\](https://github.com/oguarni/status-point.git)  
   cd status-point

2. Subir a aplicação:  
   Execute o comando na raiz do projeto. Isso irá construir as imagens do Frontend e Backend e subir o banco PostgreSQL.  
   docker-compose up \--build \-d

3. Popular o Banco de Dados (Seed):  
   Para criar os usuários padrão (admin, gestor) e dados de teste:  
